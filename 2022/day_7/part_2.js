...
// get amount we must remove to have enough space for the update
const remove = 30000000 - (70000000 - directories[directories.length - 1].total);

// create an array of directories with the needed memory space
let del = [];
directories.forEach(dir => {
    if (dir.total >= remove) del.push(dir.total);
});

// getting the smallest of the list
