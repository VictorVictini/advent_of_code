...
do {
    ...
    let notBlocked = true;
    do {
        if (sandCoords.y == largestY + 1) break;
        if (grid.findIndex(spot => spot.x == sandCoords.x && spot.y == sandCoords.y + 1) == -1) { // if path directly below is not blocked
            sandCoords.y++;
        } else if (grid.findIndex(spot => spot.x == sandCoords.x - 1 && spot.y == sandCoords.y + 1) == -1) { // if path diagonally left is not blocked
            sandCoords.x--;
            sandCoords.y++;
        } else if (grid.findIndex(spot => spot.x == sandCoords.x + 1 && spot.y == sandCoords.y + 1) == -1) { // if path diagonally right is not blocked
            sandCoords.x++;
            sandCoords.y++;
        } else {
            notBlocked = false;
...
} while (grid.findIndex(spot => spot.x == 500 && spot.y == 0) == -1);
